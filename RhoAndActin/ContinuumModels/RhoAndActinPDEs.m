...
dt=0.1;
postproc=0; % computes cross correlation if set to 1
seed=2;
rng(seed);
MakeMovie=1;
koff0=Params(1);
...
if (Dv>Du)
    ...
    u = uIC;
    v = sum(vIC,3)*2*pi/8;
    % u = ICRange(1,1)+0.5*(1+sin(2*pi*xg/L).*sin(2*pi*yg/L))...
    %    *(ICRange(1,2)-ICRange(1,1));
    % v = ICRange(2,1)+0.5*(1+sin(2*pi*xg/L).*sin(2*pi*yg/L))...
    %    *(ICRange(2,2)-ICRange(2,1));
end
...
for iT=1:nSt
    ...
    if (mod(iT,10)==0 && MakeMovie)
        ...
        clim([0 1.2])
        colormap(ax1,sky);
        title(sprintf('Rho; $t= %1.1f$',iT*dt-140))
        pbaspect([1 1 1])
        xlabel("$x$ ($\mu$m)")
        ylabel("$y$ ($\mu$m)")
        ax2=nexttile;
        ...
        clim(ICRange(2,:))
        clim([6 25])
        title(sprintf('F-actin; $t= %1.1f$',iT*dt-140))
        xlabel("$x$ ($\mu$m)")
