...
PoreSize=[];
ds=4e-3;
FullLifetime = 8; % the average lifetime of a filament
GrowAmt = 0.1/ds*dt;% (per time step - first number is um/s)
ShrinkAmt = 0.5/ds*dt; % number of monomers per time step 
MaxLength = 1;
ICScale = 0.1; 
gw=0.1;
ForceFactor=0.4/gw;
...
%tic
fg = SpreadToGrid(x,y,Xf,ForceFactor*ds*ones(sum(nPerFil),1),gw);    % units 1/area
%toc
...
for iT=1:nSt
   ...
   % uc = 1-sum(u)*dx^2;
    ...
    if (sum(abs(iT*dt-tsaves)<1e-10)>0)
        ...
        PlotXfs{index}=reshape(Xf-floor(Xf/L)*L,[],1);
        PlotTs(index)=iT*dt;
    end
    if (mod(iT-1,10)==0)
